...
let cachedParsedIPs;
let cacheExpirationTime;
const CACHE_DURATION = 1 * 60 * 1000; // 缓存有效期5分钟，可按需调整

function 解析IP集合() {
    if (cachedParsedIPs && cacheExpirationTime && Date.now() < cacheExpirationTime) {
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (!upgradeHeader || upgradeHeader !== "websocket") {
        ...
        switch (url.pathname) {
          case `/${Pswd}`: {
            const troConfig = gettroConfig(Pswd, request.headers.get("Host"));
            return new Response(`${troConfig}`, {
              status: 200,
              headers: {
        ...
        return await troOverWSHandler(request);
		}
    } catch (err) {
...
async function troOverWSHandler(request) {
...
async function parseTroHeader(buffer) {
...
function gettroConfig(Pswd, hostName) {
const parsedIPs = 解析IP集合();
  const ptrowstls = `trojan://${Pswd}@www.visa.com.sg:8443?security=tls&type=ws&host=${hostName}&sni=${hostName}&fp=random&path=%2F%3Fed%3D2560#${hostName}`;
  const note = `1、删除非tls功能(不想裸奔）\n2、删除内置域名节点\n3、新增ADDAPI远端IP\n修改自勇哥trojan(感谢勇哥👍🏻)\n\nProxyIP全局运行中：${proxyIP}`;
  const pty = `https://${hostName}/${Pswd}/pty`
  const pcl = `https://${hostName}/${Pswd}/pcl`
  const psb = `https://${hostName}/${Pswd}/psb`

  const nodeConfigs = parsedIPs.map(ip => 
    `trojan://${Pswd}\u0040${ip.host}:${ip.port}?encryption=none&security=tls&sni=${hostName}&fp=randomized&type=ws&host=${hostName}&path=%2F%3Fed%3D2560#${ip.comment}`
  ).join('\n');
  const pgtroshare = utf8ToBase64(nodeConfigs);
...
<div class="container">
    <div class="row">
        <div class="col-md-12">
            <h1>trojan代理脚本 2025.2.21</h1>
			<hr>
            <p>${noteshow}</p>
			<hr>
            <br>
            <br>
            <h3>1：Cloudflare/自定义域-trojan+ws+tls节点</h3>
			<table class="table">
				<thead>
					<tr>
						<th>节点特色：</th>
				...
				<tbody>
					<tr>
						...
						<td class="limited-width">${ptrowstls}</td>
						<td><button class="btn btn-primary" onclick="copyToClipboard('${ptrowstls}')">点击复制链接</button></td>
					</tr>
				</tbody>
			</table>
            <h5>客户端参数如下：</h5>
			...
			<h3>2：复制订阅链接：</h3>
			<hr>
			<p>注意：部分客户端通用b64链接(如:nekbox支持b64、Clash)</p>
			<hr>
			...
			<table class="table">
					...
					<tbody>
						<tr>
							<td><button class="btn btn-primary" onclick="copyToClipboard('${pgtroshare}')">点击复制链接</button></td>
						</tr>
					</tbody>
				</table>
...
function getptyConfig(Pswd, hostName) {
  ...
  return utf8ToBase64(nodeConfigs);
}


function getpclConfig(Pswd, hostName) {
    const parsedIPs = 解析IP集合();
    const proxies = parsedIPs.map((ip) => `
    - name: ${ip.comment}
      type: trojan
      server: ${ip.host}
      port: ${ip.port}
    ...
    - name: 负载均衡
      ...
      interval: 300
      proxies:
${parsedIPs.map((ip) => `        - ${ip.comment}`).join('\n')}
    - name: 自动选择
      type: url-test
      url: http://www.gstatic.com/generate_204
      interval: 150
